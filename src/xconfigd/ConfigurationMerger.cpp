...
/**
 * Class used to merge the different configuration trees generated by
 * the YAML parsers.
 *
 * The XConfigBuckets from each parser reference the nodes only from
 * their blob so in each blob the full field is used as the nodeId.
 *
 * To be able to reference nodes from other blobs the
 * nodeIds can have a canonical form, which is
 * blobId(16bit)|nodeId(16bit) (effectively limiting the number of
 * nodes for each blob to 65535)
 */
...
/**
 * Calculate the checksum for a node.
 *
 * Calculates a 64 bit checksum for a node including its data.
 *
 * For scalar values the checksum is calculated on:
 * TYPE (1byte) | VALUE (string, or in-memory representation of data)
 *
 * In case of lists:
 * TYPE | CHECKSUM(Child1) (8byte) | ... CHECKSUM(ChildN) (8byte)
 *
 * In case of maps:
 * TYPE | NAME_CHILD1 (string) | CHECKSUM(Child1) | ... NAME_CHILDN | CHECKSUM(ChildN)
 *
 * The checksum values for each node are stored in the two uint32_t
 * timestamp structures of the XConfigBucket
 */
uint64_t ConfigurationMerger::checksumNode(size_t idx) {
  bool isMap = true;
  uint64_t checksum = 0;
  XConfigBucket* cb = &destBuckets[idx - 1];
  checksum = crc64_byte(checksum, cb->type);

  switch(cb->type) {
  case xconfig::TYPE_NULL:
    // no value to checksum
    TTRACE("NULL %lx", checksum);
    break;
  case xconfig::TYPE_BOOLEAN:
    // checksum the boolean as a byte
    checksum = crc64_byte(checksum, cb->value._boolean);
    TTRACE("BOOLEAN %d %lx", cb->value._boolean, checksum);
    break;
  case xconfig::TYPE_INTEGER:
    // checksum the raw in-memory data
    checksum = crc64(checksum, (const char *)&cb->value._integer, sizeof(cb->value._integer));
    TTRACE("INTEGER %ld %lx", cb->value._integer, checksum);
    break;
  ...
  case xconfig::TYPE_STRING: {
    ...
    TTRACE("STRING %s %lx", value, checksum);
    break;
  }
  ...
  case xconfig::TYPE_MAP: {
    size_t childId = cb->value._vectorial.child;
    size_t n;
    for (n = 0; n < cb->value._vectorial.size && childId; n++) {
      uint64_t childChecksum = checksumNode(childId);
      XConfigBucket* childBucket = &destBuckets[childId - 1];
      if (isMap && childBucket->name) {
        const char* name = &destStringPool[childBucket->name - 1];
        checksum = crc64_string(checksum, name);
        TTRACE("CHILDNAME %s %lx ", name, checksum);
      }
      checksum = crc64(checksum, (const char *) &childChecksum, sizeof(childChecksum));
      TTRACE("CHILD %lx %lx ", childChecksum, checksum);
      childId = childBucket->next;
    }
    break;
  }
  default:
    // do nothing here - no other types should be in the final dump
    break;
  }

  //store into the timestamp fields
  cb->mtimeSecs = checksum >> 32;
  cb->mtimeNsecs = checksum & 0xFFFFFFFF;

  return checksum;
}

/**
 * Recursively TDEBUG the whole configuration tree
 */
void ConfigurationMerger::debugNode(size_t idx, int indent) {
  XConfigBucket* cb = &destBuckets[idx - 1];
  ...
  const char * name = "";
  if (cb->name) {
    name = &destStringPool[cb->name - 1];
  }
  sprintf(cs, "%.8x%.8x", cb->mtimeSecs, cb->mtimeNsecs);

  switch(cb->type) {
  case xconfig::TYPE_NULL:
    TDEBUG("%sNULL %s %s", prefix.c_str(), name, cs);
    break;
  case xconfig::TYPE_BOOLEAN:
    TDEBUG("%sBOOL %s %d %s", prefix.c_str(), name, cb->value._boolean, cs);
    break;
  case xconfig::TYPE_INTEGER:
    TDEBUG("%sINT  %s %ld %s", prefix.c_str(), name, cb->value._integer, cs);
    break;
  case xconfig::TYPE_FLOAT:
    TDEBUG("%sFLOT %s %g %s", prefix.c_str(), name, cb->value._float, cs);
    break;
  case xconfig::TYPE_STRING: {
    const char *value = &destStringPool[cb->value._string - 1];
  ...
  case xconfig::TYPE_MAP: {
    if (isMap) {
      TDEBUG("%sMAP  %s %s", prefix.c_str(), name, cs);
    } else {
      TDEBUG("%sSEQ  %s %s", prefix.c_str(), name, cs);
    }
    size_t childId = cb->value._vectorial.child;
    size_t n;
    for (n = 0; n < cb->value._vectorial.size && childId; n++) {
      debugNode(childId, indent + 1);
      XConfigBucket* childBucket = &destBuckets[childId - 1];
      childId = childBucket->next;
    }
    break;
...
/**
 * All configuration trees are merged into the first blob.
 * Overrides are applied in order, nodes of type delete are deleted,
 * but references remain as they are.
 */
...
/**
 * The dump function serializes the resulting configuration tree to a
 * file that can be used by the clients.
 *
 * For this first all Refs (ExpandString and ExpandRef) are expanded
 * and then all nodes are serialized into the destBuckets list.
 * All strings are stored in the destStringPool (concatenated and
 * including \0). References to string refer to the index in the
 * destStringPool and references to XConfigBuckets refer to the index
 * in the destBuckets list.
 *
 * The whole tree is checksummed and the checksum for each node stored
 * in the timestamp fields.
 *
 * Once the tree has been serialized a perfect hash function is
 * calculated for the list of keys and everything is written to a
 * file in the following format:
 *
 * XConfigHeader | CMPHData | XConfigBuckets | StringPool
 *
 */
