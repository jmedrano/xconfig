...
static void writeTable() {
  uint64_t polynomial = 0xc96c5795d7870f42ULL;  // reflected 0x42f0e1eba9ea3693ULL
...
// Pregenerated values for ECMA-182 polynomial
...
uint64_t crc64(uint64_t crc, const char* data, size_t size) {
  // to be a correct crc64 this should do ~crc at the start and before
  // returning in this case it's not important, so we can skip this step
  for(; size>0; size--) {
    crc = crc64_b(crc, *data);
    data++;
  }
  return crc;
}
